...
##########################################################################################
    ...
    y = np.hstack([0 * np.ones(7200), 1 * np.ones(7200),
                   2 * np.ones(7200), 3 * np.ones(7200),
                   4 * np.ones(7200), 5 * np.ones(7200),
                   6 * np.ones(7200), 7 * np.ones(7200),
                   8 * np.ones(7200), 9 * np.ones(7200), ]
                  )
    ...
    data_train1 = h1
    plt.plot(train_loss)
    plt.title('Train Loss encoder1')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.show()
    d_train = DataLoader(data_train1, batch_size=b_size, shuffle=True, drop_last=True)
    in_features = 600
    out_features = 200
    net = Autoencoder()
    optimizer = optim.Adam(net.parameters(), lr=lr)
    ...
    h2 = forward(h1)
    plt.plot(train_loss)
    plt.title('Train Loss encoder2')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.show()
    data_train2 = h2
    d_train = DataLoader(data_train2, batch_size=b_size, shuffle=True, drop_last=True)
    in_features = 200
    out_features = 100
    net = Autoencoder()
    optimizer = optim.Adam(net.parameters(), lr=lr)
